...
class MailGW:
    ...
    def _get_domain(self):
        ...
        if not domains:
            raise RuntimeError("mail.gw returned no available domains")
    ...
    def _ensure_domain(self):
        for attempt in range(1, DEFAULT_DOMAIN_RETRIES + 1):
            try:
                ...
                print(f"mail.gw domain fetch failed ({attempt}/{DEFAULT_DOMAIN_RETRIES}): {e}")
    ...
    def create_account(self, max_retries=DEFAULT_ACCOUNT_RETRIES):
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                print(f"mail.gw account create error ({attempt}/{max_retries}): {e}")
            ...
            print(
                f"mail.gw account create failed ({attempt}/{max_retries}), "
    ...
    def _get_token(self):
        ...
        if response.status_code != 200:
            reason = response.text.strip().replace("\n", " ")[:200]
            print(f"mail.gw token request failed: status={response.status_code}, body={reason}")
            return False
        self.token = response.json().get('token')
        if not self.token:
            print("mail.gw token request failed: empty token.")
            return False
