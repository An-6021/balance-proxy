...
def run_registration():
    ...
    with sync_playwright() as p:
        ...
        success = False
        try:
            # Existing extraction logic ... (already matches what's above)
            if api_key:
                print(f"\nSUCCESS! Account created.")
                print(f"Email: {mail.address}")
                print(f"Password: {mail.password}")
                print(f"API Key: {api_key}")
                
                with open("accounts.txt", "a") as f:
                    f.write(f"{mail.address}:{mail.password}:{api_key}\n")
                ...
                print(f"Email: {mail.address}")
                print(f"Password: {mail.password}")
                print("Please login manually to get the key.")
        finally:
            browser.close()
        ...
        if success:
            ...
            for f in os.listdir("."):
                if f.endswith(".png") or f.endswith(".html"):
                    if any(x in f for x in ["api_key_retry", "after_verify", "last_page", "login_error"]):
                        try: os.remove(f)
                        except: pass
        return success
