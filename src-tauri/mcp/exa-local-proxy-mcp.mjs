...
const apiUrl = (process.env.EXA_API_URL || "").trim().replace(/\/+$/, "");
const debugLogPath = (process.env.EXA_MCP_DEBUG_LOG || "").trim()
    || path.join(os.tmpdir(), "exa-mcp-debug.log");
...
if (!apiKey) {
    debugLog("fatal missing EXA_API_KEY");
    console.error("EXA_API_KEY environment variable is required.");
    process.exit(1);
}

if (!apiUrl) {
    debugLog("fatal missing EXA_API_URL");
    console.error("EXA_API_URL environment variable is required.");
    process.exit(1);
}
...
const TOOLS = [
    ...
    },
    {
        ...
        description: "Find similar web pages to a given URL.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Get contents of webpages using Exa.",
        inputSchema: {
            type: "object",
            properties: {
                ids: {
                    type: "array",
                    items: { type: "string" },
                    description: "List of Exa document IDs to get contents for"
                }
            },
            required: ["ids"]
        }
...
function toolRequest(name, args) {
    ...
    switch (name) {
        case "exa_search": {
            return { path: "/search", payload: input };
        }
        case "exa_find_similar":
            return { path: "/findSimilar", payload: input };
        case "exa_get_contents": {
            return { path: "/contents", payload: input };
        }
        default:
            return null;
    }
...
async function postToExa(path, payload) {
    ...
    debugLog(
        `postToExa path=${path} status=${response.status} elapsedMs=${Date.now() - started} bodyBytes=${Buffer.byteLength(
            rawBody,
            "utf8"
        )}`
    );
    ...
    if (!response.ok) {
        const detail =
            parsedBody && typeof parsedBody === "object"
                ? parsedBody.detail || parsedBody.error || parsedBody.message || JSON.stringify(parsedBody)
                : String(parsedBody || response.statusText || "Unknown error");
        return {
            content: [{ type: "text", text: `Exa API error (${response.status}): ${detail}` }],
            isError: true
        };
    }
...
async function handleRequest(request) {
    ...
    if (method === "tools/call") {
        ...
        try {
            const toolResult = await postToExa(mapped.path, mapped.payload);
            ...
            return result(id, {
                content: [{ type: "text", text: `Exa proxy request failed: ${message}` }],
                isError: true
            });
        }
    }
